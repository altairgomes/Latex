...
\hline
\endlastfoot
160/Neptune & 8$\pm$4 & 8$\pm$4 & 51 & 39 \\
...
PRAIA uses a bidimensional circular gaussian fit for the PSF of the objects in the images to obtain their photocenters. But in the case of Neptune, its size is bigger than 2 arcsec, which is comparable to the observational seeing. In this case a gaussian fit may not represent well the PSF of Neptune.
...
This equation is obtained integrating an homogeneous disk where every bit of the disk can be represented by a bidimensional circular gaussian fit. The parameters to be determined are the amplitude A, the dispersion $\sigma$, the center of the disk ($x_0$, $y_0$) and the radius of the disk R.

Fig. \ref{Fig:ajuste-psf} shows the first test with this PSF for the image of Neptune. The image test was taken at OPD in June 08, 2013. The image has a pixel scale of 0.177"/px, an exposure time of 5s and was observed with a B filter.
...
This test was made using the Scipy packages: Odrpack (for non-linear least square fit), Intergrate (for integration) and Special (for the error function)
