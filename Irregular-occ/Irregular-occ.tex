...
In this event, it is possible to see that the shade does not vary by much among the four maps suggesting thatthere was a good probability of observing the event. In fact, the biggest differences between the shadows of the four maps were \textcolor{red}{22s} along the shadow path and \textcolor{red}{109km} in the direction perpendicular to the shadows (see table \ref{Tab: comparison-Himalia}).
...
A total of \textcolor{red}{588} events were detected to happen between and the period presented. In table \ref{Tab: satellite-occultation} it's presented the number of stellar occultations predicted by year for each satellite.
...
Concerning the UCAC4 stars, an accuracy of about 20-50 mas in the star position would give a 15-40\% probability of observe such an event by Ananke, the smallest irregular satellite in the sample. For Himalia, the occultation would have 80-100\% probability of success. Combined with a mean accuracy in the position of 30 mas for Himalia and 100 mas for Ananke mas published in Table 2 of \cite{Jacobson2012}, the probability of success would be around 50-80\% for Himalia and $\approx$5\% for Ananke. Observations a few days before the date of occultation predicted may diminish the combined accuracy to 40-80 mas, depending on the magnitude of the objects.

The tests made with an occultation expected to happen in March 03, 2015 for Himalia showed that this event could probably be observed in case there were observers available in the shadow area. Fig. \ref{Fig: occ-Himalia} shows the maps made for 4 different offset corrections as explained in Sec. \ref{Sec: testes}. Table \ref{Tab: comparison-Himalia} shows numerically the difference between the maps. A similar test was made for an occultation of Elara in March 30, 2015. The difference between the maps in this case can be seen summarized in Table \ref{Tab: comparison-Elara}.

Continuous observations of the satellites and candidates are recommended and new numerical integrations of the orbits of the satellites are expected to reduce the respective position errors. The GAIA catalogue, which are to be released in the end of 2016 will improve the position error of the stars and will allow for the detection of occultations candidates of fainter stars, which are not present in the UCAC4 catalogue. Consequently, the astrometry of the satellites with the GAIA catalogue will improve the position errors of the satellites that will allow for better predictions and a bigger probability of observe stellar occultations by irregular satellites.
